...
def generate_data_set(theta_true, N=600, symm=False, umin=-1., umax=1., noise=False):
    ...
    for k in range(N-2):
        ...
        if noise:
            y[k+2] += np.random.normal(scale=0.00018)  # for approx. 30 dB SNR
    # build X for k=2..N-1
    X = np.zeros((N, 3))
    for k in range(N-2):
...
if __name__ == "__main__":
    ...
    X_train, y_train = generate_data_set(theta_true, umin=-100., umax=100., N=N, symm=True, noise=True)
    X_test, y_test = generate_data_set(theta_true, umin=-100., umax=100., N=1000)

    # Model initialization
    theta_base = np.array([0.61, -0.52, 4.7e-5])  # correspond to m_approx = 1.1, c_approx = 90, k-approx = 1100
    ...
    model = IO_orthogonal_augmentation(ANN_function=ANN_function, phi_function=phi_fun)
    model.initialize(thetaInit)
    model.set_training_params(adam_epochs=2500, lbfgs_epochs=500, lbfgs_memory=20)
    model.fit(X_train, y_train)

    Y_pred_train, orth_ANN, y_base_train, raw_ANN = model.predict(X_train)
    Y_pred_test, _, y_base_test, _ = model.predict(X_test)
