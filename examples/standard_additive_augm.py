...
if __name__ == "__main__":
    ...
    model = IO_augmentation(ANN_function=ANN_function, phi_function=generate_phi)
    ...
    Y_pred_train, _, _ = model.predict(u_train)
    Y_pred_test, _, _ = model.predict(u_test)
    ...
    theta_base_final = model.params[-1]
    ...
    _, _, ANN = model.predict(u_test_2)

    plt.figure()
    plt.plot(u_test_2, unmodeled_terms, label="Unmodeled terms")
    plt.plot(u_test_2, ANN, '--', label="Learning comp.")
    plt.grid()
    plt.legend()
    plt.show()
